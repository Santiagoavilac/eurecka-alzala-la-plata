...
import { saveMockPlayer } from "@/lib/player";
...
function EntrarPage() {
  ...
  function onSubmit(e: React.FormEvent) {
    e.preventDefault();
    setLoading(true);
    // TODO(backend): reemplazar por POST /api/players y obtener sesión real.
    saveMockPlayer(form);
    setTimeout(() => navigate({ to: "/rocket" }), 400);
  }

  const valid = form.name.trim() && form.phone.trim() && form.document.trim();
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-md px-4 pb-16">
        ...
        <form onSubmit={onSubmit} className="neon-border mt-8 space-y-5 rounded-2xl bg-surface p-6">
          <Field id="name" label="Nombre completo" value={form.name} onChange={(v) => setForm({ ...form, name: v })} placeholder="Juan Pérez" />
          <Field id="phone" label="Teléfono" value={form.phone} onChange={(v) => setForm({ ...form, phone: v })} placeholder="+595 9XX XXX XXX" type="tel" />
          <Field id="document" label="C.I. / Documento" value={form.document} onChange={(v) => setForm({ ...form, document: v })} placeholder="0.000.000" />

          <Button type="submit" size="lg" disabled={!valid || loading} className="h-14 w-full text-base font-black uppercase tracking-widest">
