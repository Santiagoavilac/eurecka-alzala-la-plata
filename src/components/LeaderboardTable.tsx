...
export function LeaderboardTable({ data }: { data: LeaderboardEntry[] }) {
  const columns: Column<LeaderboardEntry>[] = [
    {
      key: "rank", header: "#", sortable: true, className: "w-14",
      render: (r) => (
        <Badge
          variant="outline"
          className={
            r.rank === 1 ? "neon-border bg-primary text-primary-foreground"
            : r.rank <= 3 ? "neon-border bg-background"
    ...
    { key: "phoneMasked", header: "Teléfono", className: "hidden sm:table-cell text-muted-foreground" },
    {
      key: "bestMultiplier", header: "Mult.", sortable: true,
    ...
    { key: "date", header: "Fecha", sortable: true, className: "hidden md:table-cell text-muted-foreground" },
  ];
